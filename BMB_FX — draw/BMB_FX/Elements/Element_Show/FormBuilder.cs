...
namespace BMB_FX.Element_Show
{
    public class FormBuilder
    {
            ...
            for (int i = 0; i < tbl.el_list.Count; i++)
            {
                ...
                if (tbl.el_list[i].GetType() != typeof(Simple_Join_Element))
                {
                    ...
                    else
                    {
                        ...
                        //((DateTimePicker)fill_elements[i]).Text = "";
                ...
                if (tbl.el_list[i].fixed_val_flag)
                {
                    ...
                    if (tbl.el_list[i].GetType() == typeof (Simple_Join_Element))
                    {
                        ((ComboBox) fill_elements[i]).SelectedItem = tbl.el_list[i].fixed_val;
        ...
        {
            foreach (string v in tbl.make_Insert_Queue(agregate_parameters()))
            {
                SQL.Execute(v);
            }
            frm.Close();
