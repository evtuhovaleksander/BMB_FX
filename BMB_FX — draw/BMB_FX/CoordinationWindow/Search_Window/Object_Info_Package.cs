...
namespace BMB_FX
{
    ...
    public class Group_Operation_Project
    {
       ...
       public List<Window_Interval[]> Window_Sequences;
       


        public Group_Operation_Project(List<Window_Interval[]> windowSequences,DateTime t)
        {
        ...
        public TimeLineDay Get_Primary_Gui()
        {
            ...
            {
                tld.bool_mas[Window_Sequences[i][0].start] = true;
            }
            return tld;
        }
        ...
        {
            Window_Interval[] Sequence = Window_Sequences[project_id];
            TimeLineDay tld = new TimeLineDay();
            tld.spec_color = true;
    ...
    public class Head_Master_Class
    {
        ...
        public Group_Operation_Project Get_OperationProject(bool filter)
        {
                    ...
                    if (Operation_Windows[0][i].in_window(j + groupService.Operations[0].Length - 1))
                    {
                        ...
                        for (int k = 1; k < groupService.Operations.Count; k++)// подберем в секвенцию 2е и далее окна
                        {
                            for (int l = 0; l < Operation_Windows[k].Count; l++)//по итым окнам
                            {
                                ...
                                List<Window_Interval[]> win_sequences = new List<Window_Interval[]>();
                                for (int m = Operation_Windows[k][l].start; m <= Operation_Windows[k][l].stop; m++) // по квантам итого окна
                                {
                                    ...
                                    {
                                        if (m > Sequence[k - 1].stop)
                                        {
                                            Sequence[k] = new Window_Interval(Operation_Windows[k][l], m, m + groupService.Operations[k].Length - 1);
                                            if (k + 1 == groupService.Operations.Count)
                                            {
                                                ...
                                                for (int z = 0; z < Sequence.Length; z++)
                                                {
                                                    ...
                                                    {
                                                        if (Sequence[z].length != groupService.Operations[z].Length)
                                                        {
                                                            er = true;
                                                        }
                                                        else
                                                ...
                                                if (!er)
                                                {
                                                    Window_Interval[] cseq=new Window_Interval[Sequence.Length];
                                                    for (int n = 0; n < Sequence.Length; n++)
                                                    {
                                                        cseq[n] = Sequence[n];
                                                    }
                                                    win_sequences.Add(cseq);
                                                }
                                ...
                                Sequences.AddRange(win_sequences);
                            }
                            offset += groupService.Operations[k].Length;
                        }
            ...
            return new Group_Operation_Project(Sequences,day);
    ...
    public class Head_Container
    {
        ...
        List<Group_Operation_Project> GOP_List;
            ...
            bool stop)
        {
            if (st_index != -1)
            {
                ...
                for (int j = 0; j < GOP_List.Count; j++)
                {
                    ...
                    {
                        if (GOP_List[j].Window_Sequences[i][0].start == st_index)
                        {
                            row_count++;
                        }
                    }
                ...
                for (int t = 0; t < GOP_List.Count; t++)
                {

                    int row = 0;
                    for (int i = 0; i < GOP_List[t].Window_Sequences.Count; i++)
                    {
                        if (GOP_List[t].Window_Sequences[i][0].start == st_index)
                        {
                            Window_Interval[] seq = GOP_List[t].Window_Sequences[i];
                            int cell = 0;
                            for (int j = 0; j < GroupService.Operations.Count; j++)
                            {
                                dgv.Rows[row].Cells[cell].Value = i;
                                cell++;
                                dgv.Rows[row].Cells[cell].Value = GOP_List[t].Date.Date.ToString("yyyy-MM-dd");
                                cell++;

                                dgv.Rows[row].Cells[cell].Value = GOP_List[t].Window_Sequences[i][j].start;
                                cell++;
                                dgv.Rows[row].Cells[cell].Value = GroupService.Operations[j].ServiceName;
                                cell++;
                                ...
                                if (master)
                                {
                                    dgv.Rows[row].Cells[cell].Value = GOP_List[t].Window_Sequences[i][j].masterId;
                                    cell++;
                                }

                                if (resource)
                                {
                                    dgv.Rows[row].Cells[cell].Value = GOP_List[t].Window_Sequences[i][j].resourceId;
                                    cell++;
                                }

                                if (stop)
                                {
                                    dgv.Rows[row].Cells[cell].Value = GOP_List[t].Window_Sequences[i][j].stop;
                                    cell++;
                                }

                                if (j != GroupService.Operations.Count - 1)
                                {
                                    dgv.Rows[row].Cells[cell].Value = (GOP_List[t].Window_Sequences[i][j + 1].start -
                                                                       GOP_List[t].Window_Sequences[i][j].stop) - 1;
                                    cell++;
                                }
                            }
                            row++;
        ...
        public List<TimeLineDay> Draw_Project_GetTLD(int project_id, DateTime date)
        {
            ...
            for (int i = 0; i < GOP_List.Count; i++)
            {
                ...
                {
                    tld.Add(GOP_List[i].Draw_Project(project_id));
                }
                else
                {
                    tld.Add(GOP_List[i].Get_Primary_Gui());
