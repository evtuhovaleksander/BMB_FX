...
namespace BMB_FX
{
    ...
    public class Master_Gui
    {
        ...
        public Master_Gui(Operation operation, TabPage page )
        {
            ...
            tmp.HeaderText = "Master";
            
            this.dgv.Columns.Add(tmp);
            Column_To_ID.Add("Master", 1);
            ID_To_Column.Add(1, "Master");
            ...
            Column_To_ID.Add("Main_Index", 2);
            ID_To_Column.Add(2, "Main_Index");
            ...
            Column_To_ID.Add("MasterSelecter", 3);
            ID_To_Column.Add(3, "MasterSelecter");
            ...
            for (int i = 0; i < masters.Count; i++)
            {
                ...
                this.dgv.Rows[i].Cells[Column_To_ID["Master"]].Value = masters[i].master;
                this.dgv.Rows[i].Cells[Column_To_ID["Main_Index"]].Value = masters[i].MainInd;
                this.dgv.Rows[i].Cells[Column_To_ID["MasterSelecter"]].Value = true;
                masters[i].selected = true;
            }
            get_selected_all();
        }

        public void get_selected_all()
        {
    ...
    public class Resource_Gui
    {
        ...
        public Resource_Gui(Operation oper, TabPage page)
        {
            ...
            tmp = new DataGridViewColumn(new DataGridViewTextBoxCell());
            tmp.HeaderText = "Resource";
           
            this.dgv.Columns.Add(tmp);
            Column_To_ID.Add("Resource", 1);
            ID_To_Column.Add(1, "Resource");
            ...
            this.dgv.Columns.Add(tmp2);
            Column_To_ID.Add("ResourceSelecter", 2);
            ID_To_Column.Add(2, "ResourceSelecter");
            ...
            for (int i = 0; i < resource.Count; i++)
            {
                ...
                this.dgv.Rows[i].Cells[Column_To_ID["Resource"]].Value = resource[i].resource;
                this.dgv.Rows[i].Cells[Column_To_ID["ResourceSelecter"]].Value = true;
    ...
    public class Group_Operation_Project
    {
       ...
       public List<Sequence> Window_Sequences;
        ...
        {
            TimeLineDay tld = new TimeLineDay(Date);
        ...
        {
            Window_Interval[] Seq = Window_Sequences[project_id].S;
            TimeLineDay tld = new TimeLineDay(Seq[0].start_date);
            tld.spec_color = true;
            ...
            {
                for (int j = Seq[i].start; j <= Seq[i].stop; j++)
                {
                    tld.bool_mas[j] = true;
            ...
            for (int i = 0; i < Seq.Length - 1; i++)
            {
                ...
                for (int j = Seq[i].stop + 1; j < Seq[i + 1].start; j++)
                {
                    tld.bool_mas[j] = true;
    ...
    public class Head_Container
    {
        ...
        public TimeLineDay Draw_Project(int project_id)
        {
            Window_Interval[] Seq = ORM[project_id].S;
            TimeLineDay tld = new TimeLineDay(Seq[0].start_date);
            tld.spec_color = true;
    ...
    public class Master_Info
    {
       ...
       public string master;
        public double MainInd;
        public Master_Info(int masterId)
        {
            master_id = masterId;
            master = Master_ORM.get_master_where_id(masterId);
            MainInd = Master_ORM.get_MainInd(masterId);
        }
    }
    ...
    public class Resource_Info
    {
        ...
        public Resource_Info(int resourceId)
        {
            ...
            resource = Resource_ORM.get_res_where_id(resourceId);
        }
    }
