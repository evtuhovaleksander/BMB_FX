...
namespace BMB_FX
{
    ...
    {
        List<Quantum_Interval> busyIntervals;
        List<Quantum_Interval> freIntervals;
        ...
        public List<Quantum_Interval> get_concat()
        {
            List<Quantum_Interval>  freeIntervals = new List<Quantum_Interval>();
            ...
            {
                freeIntervals.Add(new Quantum_Interval(0,busyIntervals[0].start-1));
            }
            ...
            for (int i = 0; i < stop; i++)
            {
                if (busyIntervals[i].stop < busyIntervals[i + 1].start)
                {
                    ...
                    {
                        freeIntervals.Add(new Quantum_Interval(busyIntervals[i].stop + 1, busyIntervals[i + 1].start - 1));
                    }
                    else
                    {
                        continue;
                ...
                {
                    {
                        busyIntervals[i + 1]=new Quantum_Interval(busyIntervals[i].start,Math.Max(busyIntervals[i].stop, busyIntervals[i+1].stop));
                    }
                }
            }

            if (busyIntervals[stop].stop != 20)
            {
                freeIntervals.Add(new Quantum_Interval(busyIntervals[stop].stop+1,20));
            }
