...
namespace BMB_FX
{
    ...
    public class Window_Interval : Interval
    {
        ...
        //public Window_Interval(int start, DateTime startDate, Master_Interval masterInterval, Resource_Interval resourceInterval) : base(start, startDate)
        //{
        //    master_interval = masterInterval;
        //    resource_interval = resourceInterval;
        //}
        ...
        public static List<Window_Interval> overlap_master_and_resource(Master master, Resource resource)
        {
            ...
            for (int i = 0; i < day.double_bool_quants.Length / 2; i++)
            {
                if (day.double_bool_quants[i, 0] && day.double_bool_quants[i, 1])
                {
                    if (cur_interval == null)
                    {
                        cur_interval = new Interval(i, master.day.Date);
                    }
                }
                else
                {
                    ...
                    {
                        cur_interval.setStop(i - 1);
                        outt.Add(new Window_Interval(cur_interval.start, cur_interval.stop, master.day.Date, master.masterID, resource.resourceID));
                        cur_interval = null;
                    }
                }
            }
    ...
    public class Day
    {
        ...
        public Day(DateTime day, List<int> masterIds, List<int> resourceIds,int len)
        {
            this.day = day;
